	...
	<tbody>
		...
		<tr>
          ...
          <td class="td-manage">
          	...
          	@else
          	<a style="text-decoration:none" class="btn btn-xs   is_dele te" onclick="member_start(this,id)" href="javascript:;" title="启用" ><i class="icon-ok bigger-120"></i></a>
          	@endif
          <!-- <a onClick="member_stop(this,'10001')"  href="javascript:;" title="停用"  class="btn btn-xs btn-success"><i class="icon-ok bigger-120"></i></a>  -->
          <a title="编辑" onclick="member_edit('{{$user->staff_id}}')" href="javascript:;"  class="btn btn-xs  btn-info" ><i class="icon-edit bigger-120"></i></a> 
          <a title="删除" href="javascript:;"  onclick="member_del(this,'1')" class="btn btn-xs  btn-danger" ><i class="icon-trash  bigger-120"></i></a>
          <a onclick="member_resetpwd(this,'10001')" href="javascript:;" title="" class="btn btn-xs  btn-warning is_delete">重置密码</a>
          </td>
		</tr>
		@endforeach
      </tbody>
...
function member_resetpwd(obj,id){
    var staff_id = get_user_id($(obj));
    // 调用函数
    var res = ajax_req('/erp/resetPwd',{'staff_id':staff_id});
    if(res.status != 200){
      layer.alert(res.msg,{title: '提示框', icon:2}); exit;
    }else{
      // $(obj).parents("tr").remove();
